...
def initilialize():
    ...
    return reloj, fin_maquina_1, fin_maquina_2, total_espera_cola, total_tiempo_sistema, tiempo_ocupado_maquina, cantidad_clientes


reloj, fin_maquina_1, fin_maquina_2, total_espera_cola, total_tiempo_sistema, tiempo_ocupado_maquina, cantidad_clientes = initilialize()


print(f"{'Cliente':<8} | {'Llegada':<8} | {'Servicio':<8} | {'Inicio':<8} | {'Espera':<8} | {'Fin':<8}")
print("-" * 60)

while reloj < 10000000:
    #Llega un cliente
    intervalo = generar_interarribos()
    reloj += intervalo

    if reloj > 10000000: break   #Si llega despues que la maquina termina, no se hace nadd
...
if cantidad_clientes > 0:
    ...
    tasa_llegada = cantidad_clientes / 10000000       
    #Se chequea con 600 porque se calcula la capacidad total de las maquinas                           
    utilizacion_maquina_lavadora = tiempo_ocupado_maquina / (10000000*2)          
